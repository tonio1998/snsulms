	...
	const fetchStudentData = async () => {
		...
		try {
			...
			if (network?.isOnline) {
				...
				console.log("student response from StudentDetailsScreen", response)
	...
	const startScanLoop = async () => {
		...
		try {
			...
			const onTagDiscovered = async tag => {
				...
				if (tagId) {
					console.log("tag", CurrentUserID.current);
				...
				<SafeAreaView style={[globalStyles.safeArea, {paddingTop: 40}]}>
					<ScrollView contentContainerStyle={styles.container}>
						...
						<View style={{ alignItems: 'center' }}>
							<CText fontStyle="B" fontSize={20} style={{ textAlign: 'center', textTransform: 'uppercase' }}>
								{student.FirstName} {student.MiddleName} {student.LastName} {student.Suffix || ''}
							</CText>

							<Text style={[styles.label, { textAlign: 'center' }]}>{student.LRN}</Text>
							<Text style={[styles.label, { textAlign: 'center' }]}>
								Grade {student.YearLevel} - {student.Section}
							</Text>
						</View>
						...
						<View style={[globalStyles.cardRow, {marginBottom: 10}]}>
							...
							{/*<CButton title={'Reset Password'} type={'success'} icon={'key'} onPress={handleResetPass} />*/}
						</View>

						{student?.guardian && (
							<View style={styles.infoBox}>
								<View>
									<CText fontStyle="SB" fontSize={16} style={{ marginBottom: 5 }}>
										Guardian Info
									</CText>
									<Text style={styles.label}>
										Name: {student.guardian.FirstName} {student.guardian.MiddleName} {student.guardian.LastName}
									</Text>
									<Text style={styles.label}>Phone: {student.guardian.PhoneNumber}</Text>
									<Text style={styles.label}>Address: {student.guardian.Address}</Text>
								</View>
							</View>
						)}
						...
						{student?.student_user && (
							<View style={{
								...
								<View style={{ flex: 1 }}>
									<CText fontSize={15}>NFC Card:</CText>
								</View>
								<View style={{ flex: 1, flexDirection: 'row',
									justifyContent: 'space-between',
									alignItems: 'center', }}>
									<CText fontStyle={'SB'} fontSize={16}>{student.student_user.nfc_code || 'Not set'}</CText>
									<CButton title={''} type={'success'} icon={'add'} style={{ marginLeft: 10, padding: 2 }} onPress={handleConnectNFC} />
								</View>
							</View>
						)}
					</ScrollView>
